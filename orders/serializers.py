...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    def to_internal_value(self, data):
        """
        Convert the product data (ID, name, or slug) to a Product object.
        """
        internal_data = super().to_internal_value(data)
        product_data = internal_data.get('product')

        # Use the get_product_from_data method to resolve the product object
        product = self.get_product_from_data(product_data)

        if not product:
            raise serializers.ValidationError(f"Product with identifier '{product_data}' does not exist.")
        
        internal_data['product'] = product
        return internal_data

    def to_representation(self, instance):
        """
        Convert the Product object back to a string representation.
        """
        rep = super().to_representation(instance)
        # Return the product name (or slug or ID based on your needs)
        rep['product'] = instance.product.name  # or 'instance.product.slug' or 'instance.product.id'
        return rep
